({
    ...
    sourceSearch: function(cmp){
        ...
        action.setParams({'accSource':cmp.get("v.accountSourceSearch")});
    ...
    //批量删除
    deleteAccount : function(cmp){
        console.log('qqq');
		let selectedRows = cmp.find("datatable").getSelectedRows();
        console.log(selectedRows);
        List<Account> delAccount = new List<Account>;
        delAccount = selectedRows;
        var action = cmp.get('c.deleteAccount');
        action.setParams({deleteAcc:delAccount});
        action.setCallback(this, function (response){
            var state = response.getState();
            if (state == "SUCCESS"){
                console.log(response.getReturnValue());
